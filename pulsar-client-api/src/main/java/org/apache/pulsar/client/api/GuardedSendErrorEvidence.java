...
/** Evidence for a guarded SEND that returned a typed broker error. */
public record GuardedSendErrorEvidence(
    ...
    public GuardedSendErrorEvidence {
        ...
        if (serverErrorCode < 0) {
            throw new IllegalArgumentException("serverErrorCode must be non-negative");
        }
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof GuardedSendErrorEvidence)) {
            return false;
        }
        GuardedSendErrorEvidence that = (GuardedSendErrorEvidence) other;
        return protocolVersion == that.protocolVersion
                ...
                && serverErrorCode == that.serverErrorCode
    ...
    @Override
    public int hashCode() {
        int result = Objects.hash(protocolVersion, connectionGeneration, producerId, sequenceId, serverErrorCode);
